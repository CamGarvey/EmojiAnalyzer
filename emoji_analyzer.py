...
class EmojiAnalyzer:
    ...
    def get_average_color_text(self, emoji: str, padding=0, font_size=48, background: (int, int, int, int) = None, show=False):
        try:
            container_size = font_size + padding
            # Create a new image with a transparent background or background
            image = Image.new("RGBA", (container_size, container_size),
                              (0, 0, 0, 255) if not background else background)
